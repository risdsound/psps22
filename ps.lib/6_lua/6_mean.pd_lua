...
-- s3g 2016
-- an object for finding the mean value in a table

local Mean = pd.Class:new():register("6_mean")

function Mean:initialize(name, atoms)
--  pd.post("6_mean")
  self.inlets = 1
  self.outlets = 1
  return true
...
function Mean:in_1(sel, atoms)
      self:outlet(1, "float", {stats.mean(atoms)})
end
