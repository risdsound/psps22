...
-- s3g 2016
-- an object for finding the median value in a table

local Median = pd.Class:new():register("6_median")

function Median:initialize(name, atoms)
--  pd.post("6_median")
  self.inlets = 1
  self.outlets = 1
  return true
...
function Median:in_1(sel, atoms)
      self:outlet(1, "float", {stats.median(atoms)})
end
